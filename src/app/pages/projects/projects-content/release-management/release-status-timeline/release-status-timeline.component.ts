...
@Component({
  selector: 'app-release-status-timeline',
  templateUrl: './release-status-timeline.component.html',
  styleUrls: ['./release-status-timeline.component.scss']
})
export class ReleaseStatusTimelineComponent implements OnInit {
  releaseid: any = 0;
  ...
  getActivityLogs(taskid: any) {
    this.authService.getActivityLogs('release', taskid, 'status').subscribe({
      next: (res: any) => {
        // this.logs = res;
        this.rawLogs = res;
        this.processData();
