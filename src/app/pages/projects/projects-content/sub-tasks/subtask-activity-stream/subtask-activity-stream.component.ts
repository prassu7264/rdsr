...
@Component({
  selector: 'app-subtask-activity-stream',
  templateUrl: './subtask-activity-stream.component.html',
  styleUrls: ['./subtask-activity-stream.component.scss']
})
export class SubtaskActivityStreamComponent implements OnInit {
  ...
  getActivityLogs(taskid: any) {
    this.authService.getActivityLogs('task', taskid, 'all').subscribe({
      next: (res: any) => {
        console.log(res);
        this.logs = res;
        this.filterLogs()
